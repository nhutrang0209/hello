...
void GSPlay::Init()
{
	...
	m_score = std::make_shared< Text>(shader, font, "score: 0", TextColor::RED, 1.0f);
	m_score->Set2DPosition(Vector2(5.0f, 25.0f));

	//power
	m_power = std::make_shared< Text>(shader, font, "POWER: ", TextColor::RED, 1.0f);
	m_power->Set2DPosition(Vector2(5.0f, 70.0f));
...
void GSPlay::HandleEvents()
{
	...
	if (m_KeyPress & 1)//Handle event when key 'A' was pressed
	{
		if (m_gameField->IsIdle()) {
			if (m_gameField->power<300.0f) m_gameField->power += 1.5f;
			m_gameField->m_bar->Set2DPosition(75+ m_gameField->power/4,60);
			m_gameField->m_bar->SetSize(m_gameField->power/2,20);
		}
	}
	else {
		if (m_gameField->IsIdle()) {
